 ...
 */
export const createLansiaWithKode = async (data: CreateLansiaData): Promise<Lansia> => {
  try {
 ...
 * @throws Error jika lansia tidak ditemukan
 */
export const findMinimalLansiaByKode = async (kode: string): Promise<MinimalLansiaData> => {
  try {
    const lansia = await findLansiaByKode(kode);
